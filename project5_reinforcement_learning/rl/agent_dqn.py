...
def epsilon_greedy(state_vector, epsilon):
    ...
    if random < epsilon:
        ...
        action_index, object_index = q_values_action.argmax().item(), q_values_object.argmax().item()
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_values_action_current, q_values_object_current = q_value_cur_state 
    if terminal:
        update = reward
    else:
        update = reward + GAMMA * maxq_next
    q_value = q_values_action_current[action_index] + q_values_object_current[object_index]
    delta = q_value - update
    loss = 1/2 * delta**2 
