...
TRAINING_EP = .05  # epsilon-greedy parameter for training
TESTING_EP = 0.05  # epsilon-greedy parameter for testing
NUM_RUNS = 10
NUM_EPOCHS = 200
NUM_EPIS_TRAIN = 25  # number of episodes for training at each epoch
NUM_EPIS_TEST = 50  # number of episodes for testing
ALPHA = .00001  # learning rate for training
...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if random < epsilon:
        #explore
        action_index, object_index = np.random.choice(list(range(NUM_ACTIONS))), np.random.choice(list(range(NUM_OBJECTS)))
    else:
        #pi argmax
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    Returns:
        None
    """
    prior = q_func[current_state_1, current_state_2, action_index, object_index]
    if terminal:
        update = reward
    else:
        update = reward+GAMMA*np.max(q_func[next_state_1, next_state_2])
    q_func[current_state_1, current_state_2, action_index, object_index] = (1-ALPHA)*prior + ALPHA*update
...
def run_episode(for_training):
    ...
    epi_reward = None if for_training else 0 
    turn = 0
