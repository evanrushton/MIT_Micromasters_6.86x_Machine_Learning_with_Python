...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    Returns:
        None
    """
    prior = (theta@current_state_vector)[tuple2index(action_index, object_index)]
    if terminal:
        update = reward
    else:
        update = reward + GAMMA*np.max((theta@next_state_vector))
    theta[tuple2index(action_index, object_index)] = theta[tuple2index(action_index, object_index)] + ALPHA*(update - prior) * current_state_vector.T
# pragma: coderesponse end
